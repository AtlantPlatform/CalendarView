...
import com.kizitonwose.calendarview.ui.WeekStartEndBinder
import com.kizitonwose.calendarviewsample.databinding.*
...
class Example4_1_Fragment : BaseFragment(R.layout.example_4_1_fragment), HasToolbar, HasBackButton {
    ...
    private lateinit var binding: Example41FragmentBinding

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)
        setHasOptionsMenu(true)
        binding = Example41FragmentBinding.bind(view)
            ...
            init {
                view.setOnClickListener {
                    if (day.owner == DayOwner.THIS_MONTH && (day.date == today || day.date.isAfter(
                        ...
                        this@Example4_1_Fragment.binding.exFourCalendar.notifyCalendarChanged()
                        bindSummaryViews()
                    }
                }
            }
        ...
        class WeekViewContainer(view: View) : ViewContainer(view) {
            lateinit var day: CalendarDay // Will be set when this container is bound.
            val binding = Example4CalendarStartEndWeekBinding.bind(view)
        }

        binding.exFourCalendar.startWeekWidth = resources.getDimension(R.dimen.week_start_end_width).toInt()
        binding.exFourCalendar.endWeekWidth = resources.getDimension(R.dimen.week_start_end_width).toInt()
        binding.exFourCalendar.weekStartBinder = object : WeekStartEndBinder<WeekViewContainer> {
            override fun create(view: View) = WeekViewContainer(view)
            override fun bind(container: WeekViewContainer, day: CalendarDay) {
                container.day = day
                val roundBgView = container.binding.exFourWeekBgView
                roundBgView.makeInVisible()
                if (endDate != null && startDate != null && (day.date.isBefore(endDate) || day.date.isEqual(endDate))
                    && day.date.isAfter(startDate)) {
                    roundBgView.setBackgroundResource(R.drawable.example_4_continuous_selected_bg_middle)
                    roundBgView.makeVisible()
                }
            }
        }
        binding.exFourCalendar.weekEndBinder = object : WeekStartEndBinder<WeekViewContainer> {
            override fun create(view: View) = WeekViewContainer(view)
            override fun bind(container: WeekViewContainer, day: CalendarDay) {
                container.day = day
                val roundBgView = container.binding.exFourWeekBgView
                roundBgView.makeInVisible()
                if (endDate != null && startDate != null && day.date.isBefore(endDate)
                    && (day.date.isAfter(startDate) || day.date.isEqual(startDate))) {
                    roundBgView.setBackgroundResource(R.drawable.example_4_continuous_selected_bg_middle)
                    roundBgView.makeVisible()
                }
            }
